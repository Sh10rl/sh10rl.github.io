...
title: NCTF 2025
date: 2025-4-4
description: description 2025 Writeup
categories: [ctf]
tags: [vm, go]
---
...
> Note: Input buffer structure: 0x40(maxLength) + actualLength+ data
> 
...
> The main function implements SM4 encryption without modifying SBox. Note that one dummy thread is continuously spawned / terminated, which triggers TLS each time.
TLS Anti-debugging: TLS then triggers CRC self-validation upon every thread creation, unable to set software breakpoint(INT 3 == 0xCC). Hence, use hardware breakpoint to bypass CRC self-check, obtaining hacked SBox.
> 
...
Callback functions were found:

> DLL_PROCESS_ATTACH (1): Process initializationDLL_THREAD_ATTACH (2): Thread creationDLL_THREAD_DETACH (3): Thread exitDLL_PROCESS_DETACH (0): Process termination
> 
